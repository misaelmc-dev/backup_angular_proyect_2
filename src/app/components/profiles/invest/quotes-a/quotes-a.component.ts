...
@Component({
  selector: 'app-quotes-a',
  templateUrl: './quotes-a.component.html',
  styleUrls: ['./quotes-a.component.css']
})
export class QuotesAComponent implements OnInit {
  ...
  loadQuotes(noPaginate?: boolean): void {
    this.quoteData = [];
    this.dataSource = [];
    this.criteria = `productos/${this.productId}/citas?onlyCitesA&s=${encodeURIComponent(this.filter)}&page=${this.pageNumber}&page_size=${this.pageSize}`;
    if (noPaginate)
      this.criteria += '&no_paginate'
  ...
  exportToExcel(): void {
    ...
    let criteria = `productos/${this.productId}/citas?onlyCitesA&s=${encodeURIComponent(this.filter)}&page=${this.pageNumber}&page_size=${this.pageSize}&no_paginate`;
    this.researchService.getProductQuotesByResearcher(criteria).subscribe((quotes: any) => {
      //console.log('Quotes A: ', quotes );
      ...
      edata.push(udt);
      this.exportService.exportJsonToExcel(edata, 'Citas_A_' + this.researcherName);
  ...
  public exportToCsv() {
    const filename = 'Citas_A_' + this.researcherName
    this.showLoadingBar = true
    const separator = ','
    const keys = ['Titulo', 'DOI', 'Url', 'Anio', 'Tipo', 'Id_tipo', 'Producto', 'Autores', 'Fuente']
    let criteria = `productos/${this.productId}/citas?onlyCitesA&s=${encodeURIComponent(this.filter)}&page=${this.pageNumber}&page_size=${this.pageSize}&no_paginate`;
    this.researchService.getProductQuotesByResearcher(criteria).subscribe((dataSource: any) => {
      const csvContent =
        keys.join(separator) +
        '\n' +
