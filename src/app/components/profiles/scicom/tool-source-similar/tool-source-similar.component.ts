...
@Component({
  selector: 'app-tool-source-similar',
  templateUrl: './tool-source-similar.component.html',
  styleUrls: ['./tool-source-similar.component.css']
})
export class ToolSourceSimilarComponent implements OnInit {
  ...
  ordenSimilitud:string='sorting';
  ordenSimilitudCount:number=0;
  ...
  loadFuentes(recargarFuentes?:boolean) {
    this.loadingCount++
    this.fuentesService.getFuentes().subscribe((res: any) => {
      this.loadingCount--
      var c = res.data.sort((a:any,b:any) => { return a.titulo - b.titulo;});
      this.fuentesOrigen = c
      if(!recargarFuentes){
        this.loadPaises()
        this.loadIdiomas()
        this.loadAreas()
  ...
  filtrarFuenteOrigen(){
    var consulta = "";
    if(this.fuentesSearch!=''){
      consulta = consulta+'&search='+this.fuentesSearch
      this.fuenteSelected = 0;
    }
    this.loadingCount++
    this.fuentesService.getFuentesFiltradas(consulta).subscribe((res: any) => {
      this.loadingCount--
      this.fuentesOrigen = res.data
    }, (err:any) =>{
      this.loadingCount--
  ...
  filtrarFuentesSimilares(value?:string){
    ...
    if(this.fuenteSelected!=0){
      ...
      var consulta = "&idFuentOrigSimilaridad="+this.fuenteSelected;
  ...
  ordenarFuentes(value:string){
    if(this.fuentesList.length!=0){
      ...
      if(value=='cites'){
        ...
        this.ordenSimilitud='sorting'
        this.ordenSimilitudCount=0
      }
      if(value=='quartil'){
      ...
      if(value=='similitud'){
        if(this.ordenSimilitudCount==0){
          this.ordenSimilitud='sorting_asc'
          this.ordenSimilitudCount=1
          /*
          this.fuentesList = this.fuentesList.sort((a:any,b:any) => {
            if(a.fuentes_similares_originarias[0].pivot.similarity>b.fuentes_similares_originarias[0].pivot.similarity){return 1}
            if(a.fuentes_similares_originarias[0].pivot.similarity<b.fuentes_similares_originarias[0].pivot.similarity){ return -1}
            return 0
          });
          this.fuentesList = auxFuentesList.sort((a,b)=>{if(a.cites_score==null){return -1}return 0});
          */
          ...
          this.ordenSimilitud='sorting_desc'
          this.ordenSimilitudCount=0
          /*
          auxFuentesList = auxFuentesList.sort((a,b)=>
            b.fuentes_similares_originarias[0].pivot.similarity-a.fuentes_similares_originarias[0].pivot.similarity
          );
          this.fuentesList = auxFuentesList.sort((a,b)=>{if(b.cites_score==null){return -1}return 0});
          */
        }
  ...
  cleanOrigenFilters(){
    ...
    var sinRecargar:boolean = true
    this.loadFuentes(sinRecargar)
    this.filtrarFuentesSimilares()
    this.cleanFilters()
  }
