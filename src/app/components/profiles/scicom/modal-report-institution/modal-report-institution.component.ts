...
import {SidebarService} from "../../../../services/scicom/sidebar.service";
...
@Component({
  selector: 'app-modal-report-institution',
  templateUrl: './modal-report-institution.component.html',
  styleUrls: ['./modal-report-institution.component.css']
})
export class ModalReportInstitutionComponent implements OnInit {
  ...
  loadEvents(){
    this.loadingCount++
    this.sidebarService.getEventListWidthId().subscribe((resEventList: any) => {
      this.loadingCount--
  ...
  loadInstitucionesEvent(idsEvento: Array<number>) {
    if (idsEvento.length > 0){
      ...
      this.loadingCount++
      this.sidebarService.getEventsByType([2,3]).subscribe((resInstitucion: any) => {
        this.loadingCount--
        ...
        if (idsRorInstitucionesCreadorasEvento.length > 0) {
          let idsRors = idsRorInstitucionesCreadorasEvento.map((value: any) => {return value.idRor})
          this.loadingCount++
