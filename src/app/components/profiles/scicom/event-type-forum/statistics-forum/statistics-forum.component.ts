...
@Component({
  selector: 'app-statistics-forum',
  templateUrl: './statistics-forum.component.html',
  styleUrls: ['./statistics-forum.component.css']
})
export class StatisticsForumComponent implements OnInit {
  ...
  loadTiposEvento(){
    this.estadisticService.getTiposEvento(this.cordId).subscribe((resTipEve:any) => {
      for(let rte of resTipEve){
        if(rte.codigo_interno=="tipEvForoUvm"){this.typeEventId=rte.id}
      }
