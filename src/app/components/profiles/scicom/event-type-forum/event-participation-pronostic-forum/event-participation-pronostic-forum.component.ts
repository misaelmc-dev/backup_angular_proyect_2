...
@Component({
  selector: 'app-event-participation-pronostic-forum',
  templateUrl: './event-participation-pronostic-forum.component.html',
  styleUrls: ['./event-participation-pronostic-forum.component.css']
})
export class EventParticipationPronosticForumComponent implements OnInit {
  ...
  savePronosticsCarteles(){
    if(this.cartelesValorObjetivo!=0 && this.cartelesValorObjetivo!=undefined) {
      ...
      this.cartelesRealValueUrl = "/api/scicom/trabajos/statistics?idsPart=" + this.participacionId + "&idsTipsTrab=" + encodeURIComponent(this.tiposTrabajo.join(' ')) + "";
      this.loadingCount++
  ...
  savePronosticsEstudiantes(){
    if(this.estudianteValorObjetivo!=0 && this.estudianteValorObjetivo!=undefined){
      ...
      this.estudianteRealValueUrl="/api/scicom/autorestrabajo/statistics?no_paginate=&idsPart="+this.participacionId+"&idsTipsTrab="+encodeURIComponent(this.tiposTrabajo.join(' '))+"";
      this.loadingCount++
