...
@Component({
  selector: 'app-event-new-forum',
  templateUrl: './event-new-forum.component.html',
  styleUrls: ['./event-new-forum.component.css']
})
export class EventNewForumComponent implements OnInit {
  ...
  ngOnChanges(changes:SimpleChanges){
    ...
    this.variableForo=changes['variableForo'].currentValue;
    //console.log("Encuentro:",this.variableForo);
    if(this.eventoId==0){
      this.validarFormulario()
    }
    if(this.variableForo=='actualizar'){
      this.validarFormulario()
    }
    if(this.variableForo=='actualizarCerrar'){
      this.validarFormulario()
    }
  }
  ...
  validacionCompleta(){
    if(this.validacionNombre==0 &&
      ...
      if(this.variableForo=='guardar'){
        this.crearEvento()
      }else if(this.variableForo=='guardarCerrar'){
        this.crearEvento()
      }else if(this.variableForo=='actualizar'){
        this.actualizarEvento()
      }else if(this.variableForo=='actualizarCerrar'){
        this.actualizarEvento()
      }
    }else{
