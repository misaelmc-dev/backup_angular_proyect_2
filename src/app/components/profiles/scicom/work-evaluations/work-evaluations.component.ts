...
@Component({
  selector: 'app-work-evaluations',
  templateUrl: './work-evaluations.component.html',
  styleUrls: ['./work-evaluations.component.css']
})
export class WorkEvaluationsComponent implements OnInit {
  //idEvento: number = null //todo // id de evento, imprescindible que entre a este componente
  /*eventoId = Number(this.route.snapshot.paramMap.get('idevent'));
  idTipoEvaluacion = Number(this.route.snapshot.paramMap.get('idtipeval'));*/
  @Input('eventoId') eventoId: any
  @Input('idTipoEvaluacion') idTipoEvaluacion: any
  ...
  usuariosPreevaluadores: any = [];

  idTrabajoUpdate:number = 0;
  ...
  loadPreevaluaciones(){
    if(this.idTipoEvaluacion==2){
      ...
      suscripcionTrabajos.subscribe((res: any) => {
        ...
        let arrayTiposprevaluaciones = [1];
        ...
        preevaluacionesSuscripcion.subscribe((resPrev:any) => {
          this.loadingCount--
          this.prevaluacionesTrabajos = resPrev;
          let auxprevaluaciones = this.newObjectReference(this.prevaluacionesTrabajos);
          for(let e1 of this.prevaluacionesTrabajos){
            for(let e2 of this.prevaluacionesTrabajos){
              if(e1.id_trabajo == e2.id_trabajo){
                if(moment(e1.update_time) == moment(e2.update_time)){
                  ...
                  auxprevaluaciones = auxprevaluaciones.filter((value: any) => value.id != e1.id);
                }else if(moment(e1.update_time) > moment(e2.update_time)){
                  auxprevaluaciones = auxprevaluaciones.filter((value: any) => value.id != e2.id);
                }
              }
            }
          }
          this.prevaluacionesTrabajos = auxprevaluaciones;
          this.loadTrabajosAlInicio();
        }, (err:any) =>{
          this.loadingCount--
  ...
  loadTrabajosAlInicio() {
    let suscripcionTrabajos = this.cargaTrabajosSuscripcion(this.eventoId)
    this.loadingCount++
    suscripcionTrabajos.subscribe((res: any) => {
      this.loadingCount--
      let items:any = [] ;
      if(this.prevaluacionesTrabajos.length!=0){
        let auxprev = this.prevaluacionesTrabajos;
        for (let item of res) {
          item.preevaluaciones = []
          auxprev = this.prevaluacionesTrabajos.filter((value: any) => { return value.id_trabajo == item.id})
          if(auxprev.length!=0){
            item.preevaluacion = auxprev[0].aprobatoria;
            item.preevaluaciones.push(auxprev[0]);
          }
        }
        let auxPrev = res;
        for(let item of res){
          if(!item.preevaluacion){
      ...
      suscripcionConjuntoMetadatos.subscribe((res:any) => {
        ...
        for (let item of this.items) {
          let auxValidado:boolean=false;
          for(let im of item.metadatos){
            if(im.productos_count!=0){
              auxValidado = true
            }
          }
          item.validado = auxValidado;
        }
      }, (err:any) =>{
        this.loadingCount--
      ...
      evaluacionesSuscripcion.subscribe((res:any) => {
        ...
        if(this.idTipoEvaluacion==2){
          this.loadUsuariosPreevaluacion(this.prevaluacionesTrabajos);
        }
        let arrayTiposArchivo = [6];
        let evaluacionesArchivosSuscripcion = this.cargaArchivosEvaluacionSuscripcion(this.eventoId,arrayTiposArchivo);
        this.loadingCount++
        evaluacionesArchivosSuscripcion.subscribe((res:any) => {
          this.loadingCount--
          for (let item of this.items) {
            if(item.evaluaciones.length!=0){
              if(item.evaluaciones[0].id){
                let auxArchivosEvaluacion = this.newObjectReference(res);
                item.archivosEvaluacion = auxArchivosEvaluacion.filter((value: any) => { return value.id_evaluacion == item.evaluaciones[0].id })
              }
            }
          }
        }, (err:any) =>{
          this.loadingCount--
          Swal.fire({icon: 'error',text: 'Error al cargar los archivos de evaluaciones de los trabajos especificados'});
          console.error(err)
        });
  ...
  loadUsuariosPreevaluacion(preevaluaciones:any){
    //inicia valida que los ids de usuarios en las evaluaciones no reciban id igual a null
    //todo si el id_usuario es nulo en una evaluación, el usuario que la creo tiene el rol de admin o cord
    let auxPreevaltrab:any = preevaluaciones;
    auxPreevaltrab = auxPreevaltrab.filter((value: any) => value.id_usuario != null)
    //Fin valida que los ids de usuarios en las evaluaciones no reciban id igual a null
    const idsUsuarios = auxPreevaltrab.map((value: any) => value.id_usuario)
    if(idsUsuarios.length != 0){
      let suscripcionUsuariosEvaluacion = this.cargaUsuariosScicomSuscripcion(this.eventoId,idsUsuarios)
      this.loadingCount++
      suscripcionUsuariosEvaluacion.subscribe((res) => {
        this.loadingCount--
        this.usuariosPreevaluadores = res;
        let auxusuario = this.newObjectReference(res)
        for (let item of this.items) {
          for(let ev of item.preevaluaciones){
            ev.userPreeval = auxusuario.filter((value: any) => value.id == ev.id_usuario)
          }
        }
      }, (err:any) =>{
        this.loadingCount--
  ...
  filtrarTrabajos() {
    ...
    if (this.selectedDictamen != 0) this.filtraPorDictamen(this.selectedDictamen)
  ...
  filtraPorDictamen(dictamenId: any) {
    const auxTrab = this.itemsToShow.filter((value: any) => {
      if (dictamenId == '1') return (value.evaluaciones.length == 0)
      const aux = value.evaluaciones.filter((value: any) => {
        switch(dictamenId) {
          case '2': return value.aprobatoria //aprobado
          case '3': return !value.aprobatoria //no aprobado
        }
      })
      return aux.length > 0
    })
  ...
  agregarArchivo(){
    ...
    this.archivosEvaluacion.push({nombre:this.nombreArchivo,descripcion:this.descripcionArchivo,file:this.archivoTrabajo,clave:timestamp})
  }

  aprobadoFileModal(id_trabajo:any){
    this.idTrabajoUpdate = id_trabajo;
    this.idEvaluation = 0;
    this.archivosEvaluacion = [];
    this.archivosEvaluacionOld = [];
    this.modalDisabled = true;
    this.evaluacionTrue = false;
    this.evaluacionFalse = false;
    for(let itemsts of this.itemsToShow){
      if(itemsts.id == id_trabajo){
        if (itemsts.evaluaciones[0].aprobatoria){
          this.evaluacionTrue = true;
        }else{
          this.evaluacionFalse = true;
        }
        this.idEvaluation = itemsts.evaluaciones[0].id;
        //console.log("this.idEvaluation",this.idEvaluation)
        this.dictamen = itemsts.evaluaciones[0].descripcion;
        this.archivosEvaluacion = itemsts.archivosEvaluacion;
        this.archivosEvaluacionOld = this.newObjectReference(itemsts.archivosEvaluacion);
      }
    }
  }

  noaprobadoFileModal(id_trabajo:any){
    this.idTrabajoUpdate = id_trabajo;
    this.idEvaluation = 0;
    this.archivosEvaluacion = [];
    this.archivosEvaluacionOld = [];
  ...
  sinEvaluarFileModal(id_trabajo:any){
    this.archivosEvaluacion = [];
  ...
  saveEvaluation(idTrabajo:any){
    let evalua:number = 0;
    if(this.evaluacionTrue){evalua = 1;}
    this.loadingCount++
    this.eventService.addEvaluacionToWork(this.idTipoEvaluacion,this.dictamen,this.eventoId,this.idTrabajoGuardar,evalua).subscribe((res: any) => {
      this.loadingCount--
      this.loadingCount++
      this.notificacionesScicomService.sendNotificationAddEvalTrabEvento(this.eventoId,res).subscribe((res:any) => {
        this.loadingCount--
        console.info("NotificationAddEvalTrabEvento")
      }, (err:any) =>{
        this.loadingCount--
        console.error(err)
      });
      for(let archivo of this.archivosEvaluacion){
        this.saveFileToEvaluation(archivo.nombre,archivo.descripcion,res,archivo.file)
        this.loadTrabajosAlInicio()
      }
      ...
      this.loadingCount--
      Swal.fire({icon: 'error',text: 'Error guardar la evaluación al trabajo especificado'});
      console.error(err)
    });
  }

  updateEvaluation(idEvaluation:number){
    for(let archivo of this.archivosEvaluacion){
      if(archivo.clave){
        this.saveFileToEvaluation(archivo.nombre,archivo.descripcion,idEvaluation,archivo.file)
        this.loadTrabajosAlInicio()
      }
    }
  }
  ...
  saveFileToEvaluation(nombre:string,descripcion:string,evaluationId:number,file:any) {
    this.loadingCount++
    this.eventService.addFileEvaluation(nombre, descripcion, this.eventoId, 6, evaluationId, file).subscribe((res: any) => {
      this.loadingCount--
  ...
  removeArchivoByClave(clave:any){
    this.archivosEvaluacion = this.archivosEvaluacion.filter( (af:any) => af.clave != clave)
  }
