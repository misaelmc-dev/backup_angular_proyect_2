...
export class MyZoneComponent implements OnInit, AfterViewInit {
  ...
  loadData = () => {
    ...
    this.criteria = `myzone/productos?id_institucion=${this.id}&page=${this.pageNumber}&page_size=${this.pageSize}&anios=${this.anios}&search=${this.search}&idsPaises=${this.idsPaises}&idsLineas=${this.idsLineas}&idsTiposProducto=${this.idsProductos}&idsInstituciones=${this.idsInstituciones}&idsColegios=${this.idsColegios}&idsCampus=${this.idsCampus}&idsInvestigadores=${this.idsInvestigadores}`;
   ...
   */
  loadVisibilidades() {
    this.productosService.getVisibilidadesConProductos(null, this.idsInstitucionesArray)
      .subscribe((data: any) =>
      {
        console.debug('Visibilidades productivas cargados en mi producción de coordinador: ' + data)
  ...
  loadYears = () => {
      this.years = [];
      this.productosService.getAniosProductivos(this.idsInstitucionesArray).subscribe((anios: any) => {
        console.debug('Anios productivos cargados en mi producción de coordinador: ' + anios)
        for(let a of anios) {this.years.push({year:a,checked:''});}
      },() => {
        console.error('Error al cargar los años productivos en mi producción de coordinador')
      })
  }
  ...
  loadLines = () => {
    ...
    this.lineasInvestigacionService.get(null, null, null, null, null,
      [this.id], null, null, true).subscribe(
      (result: any) => {
        console.debug('Cargadas líneas de investigación para filtrar en mi producción de coordinador')
  ...
  loadProductTypes = () => {
    this.tiposDeProductoService.get(true, false, null, [this.id]).subscribe(
      (result: any) => {
  ...
  loadCampus = () => {
    ...
    this.campusService.getSimples(this.id, null, null, null, null,
      null, null, null, null, null, true).subscribe(
      (result: any) => {
        this.campus = result;
  ...
  exportToExcel(): void {
    ...
    this.adviserService.getMyZoneData(excelCriteria).subscribe((result: any) => {
      ...
      this.exportService.exportJsonToExcelMyImpact(edata, 'My_Zone_Productos_' + this.institutionName);
      this.showLoadingBar = false;
    });
  }
