...
@Component({
  selector: 'app-colab-center-destino-institucion-coord',
  templateUrl: './colab-center-destino-institucion-coord.component.html',
  styleUrls: ['./colab-center-destino-institucion-coord.component.css']
})
export class ColabCenterDestinoInstitucionCoordComponent implements OnInit {
  @Input() titulo: string = ''
  ...
  instituciones: Array<any> = []; //arreglo de instituciones
  showElemList: boolean;
  selectedInstitutionId: number //id seleccionado al hacer clic
  selectedInstitutionNombre: string //nombre seleccionado al hacer clic
  ...
  ngOnInit(): void {
    ...
    this.tiposElementosDestino.push('instituciones')
    ...
    data.forEach((item: any) => {
      if (item.desglose !== undefined) {
        ...
        this.instituciones.push(
          {
            idRorInterno: item.id_ror_interno,
            idRorOficial: item.id_ror_oficial,
  ...
  onElementClicked(id: number, nombre: string) {
    this.selectedInstitutionId = id
    this.selectedInstitutionNombre = nombre
